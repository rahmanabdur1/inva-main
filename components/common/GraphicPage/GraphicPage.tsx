...
import Box from "@common/GraphicAnim/Box";
...
function Content() {
  const images = useLoader(
    //   @ts-ignore
    TextureLoader,
    state.paragraphs.map(({ image }: any) => image)
  );

  useMemo(
    () =>
      images.forEach(
        (texture: { minFilter: number }) => (texture.minFilter = LinearFilter)
      ),
    [images]
  );
  ...
  return (
    <>
      ...
      {state.paragraphs.map((props: any, index: number) => (
        <Paragraph key={index} index={index} {...props} image={images[index]} />
      ))}
      {state.stripes.map(
        (
          { offset, color, height }: any,
